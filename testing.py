...
for num in range(len(video_names)):
    ...
    print("processing " + str(num+1) + " out of " + str(len(video_names)) + " videos")
    
    # takes in a video
    cap = cv2.VideoCapture(video_names[num])
    gaze = GazeTracking()
   ...
   # first x-coord & y-coord
    ...
    plt.subplot(4,3,num+1)
    plt.scatter(x_coord_head_only, y_coord_head_only, c=timing_head_only, cmap='winter',zorder=3)
    plt.title(video_names[num])
    plt.grid(b=True,color='#BEBEBE',zorder=0)
    plt.xlim(-60,60)
    plt.ylim(-150,150)
    ...
    plt.subplot(4,3,num+1)
    plt.scatter(x_coord_eye_only, y_coord_eye_only, c=timing_eye_only, cmap='winter',zorder=3)
    plt.title(video_names[num])
    plt.grid(b=True,color='#BEBEBE',zorder=0)
    plt.xlim(-60,60)
    plt.ylim(-150,150)
    ...
    max_x_coords=[max(x_coord),max(x_coord_eye_only),max(x_coord_head_only)]
    max_y_coords=[abs(min(y_coord)),abs(min(y_coord_eye_only)),abs(min(y_coord_eye_only))]
